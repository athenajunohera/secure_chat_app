...
router.post('/register', async (req, res) => {
    try {
        const { username, password, publicKey, avatar } = req.body;
        const items = await User.findOne({ username });
        if (items) return res.status(400).json({ message: 'User already exists' });

        const hashedPassword = await bcrypt.hash(password, 10);
        const newUser = new User({ username, password: hashedPassword, publicKey, avatar });
        await newUser.save();
...
router.post('/login', async (req, res) => {
    try {
        ...
        res.json({ token, username, publicKey: user.publicKey, avatar: user.avatar });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
